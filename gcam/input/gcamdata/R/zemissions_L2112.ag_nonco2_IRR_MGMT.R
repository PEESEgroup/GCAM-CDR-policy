...
module_emissions_L2112.ag_nonco2_IRR_MGMT <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L2112.AWBEmissions %>%
      ...
      # Cayuela, M.L., Van Zwieten, L., Singh, B.P., Jeffery, S., Roig, A. and S?nchez-Monedero, M.A., 2014. Biochar's role in mitigating soil nitrous oxide emissions: A review and meta-analysis. Agriculture, Ecosystems & Environment, 191, pp.5-16.
      mutate(input.emissions = if_else(level=="biochar" & Non.CO2 == "N2O_AGR"& (kg_bio_ha > aglu.BIOCHAR_LOWER_APP_RATE), input.emissions*.98, input.emissions)) %>%
