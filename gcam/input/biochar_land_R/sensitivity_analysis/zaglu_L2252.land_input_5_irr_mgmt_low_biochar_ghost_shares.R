...
module_aglu_L2252.land_input_5_irr_mgmt <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L171.ag_EcYield_kgm2_R_C_Y_GLU %>%
      ...
      add_carbon_info(carbon_info_table = L121.CarbonContent_kgm2_R_LT_GLU) %>% # in terms of kg C/m2
      ...
      mutate(hist.veg.carbon.density = round(value / (HarvestIndex.x) * (1 + Root_Shoot.x) * (1 - WaterContent.x) *
    ...
    L2252.LN5_MgdAllocation_bio %>%
      ...
      select(-landshare) %>%
      # For biochar techs, set ghost-share to 0.01, or 1%
      replace_na(replace = list(ghost.unnormalized.share = 0.01)) %>%
