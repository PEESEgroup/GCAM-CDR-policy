...
module_aglu_L2062.ag_Fert_irr_mgmt <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L2062.AgCoef_Fert_ag_irr_mgmt %>%
      ...
      mutate(coefficient = if_else(grepl("biochar", AgProductionTechnology) & (kg_bio_ha > aglu.BIOCHAR_LOWER_APP_RATE) & (minicam.energy.input == "N fertilizer"), coefficient*1, coefficient)) %>%
      select(-kg_bio_ha)->
      L2062.AgCoef_Fert_ag_irr_mgmt
