...
#' \code{L202.StubTechProd_an}, \code{L202.StubTechCoef_an}, \code{L202.StubTechCost_an}, \code{L202.StubTechFractSecOut_an}, \code{L202.Rsrc_an}
...
#' @author Nathan Preuss August 2023 & BBL August 2017
module_aglu_L202.an_input <- function(command, ...) {
  ...
  MODULE_INPUTS <-
    c(FILE = "common/GCAM_region_names",
      ...
      FILE = "aglu/A_an_secout",
      FILE = "aglu/A_an_rsrc_info",
      FILE = "aglu/A_an_manure_tech",
      FILE = "aglu/A_an_secout_prices",
  ...
  MODULE_OUTPUTS <-
    c("L202.RenewRsrc",
      ...
      "L202.Rsrc_an",
      "L202.StubTechFractProd_an",
      "L202.StubTechFractCalPrice_an",
      "L202.RsrcPrice_an",
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L202.manure_types_region <- filter(A_an_manure_tech) %>%
      repeat_add_columns(GCAM_region_names) %>%
      select(region, supplysector, subsector, technology)

    print(L202.manure_types_region)

    print(A_an_secout)

    #prepare the secondary outputs table
    A_an_secout %>%
      ...
      inner_join(L202.manure_types_region, by = c("supplysector", "subsector", "technology"))->
      L202.globaltech_secout_R

    print(L202.globaltech_secout_R)

    #attach secondary output and interpolate to all years
    L202.globaltech_secout_R %>%
      gather_years() %>%
      ...
      select(region, supplysector, subsector, stub.technology = technology, fractional.secondary.output,
             output.ratio, year) -> L202.StubTechFractSecOut_an

    print(L202.StubTechFractSecOut_an)
    ...
    A_an_secout_prices %>%
      ...
      select(GCAM_region_ID, feed_price, region, secout) -> L202.an_FeedPrice_R_Yf
    ...
    L202.StubTechFractSecOut_an %>%
      ...
      select(-variable, -feed_price, -GCAM_region_ID) -> L202.StubTechFractProd_an
    ...
    L202.StubTechFractSecOut_an %>%
      ...
      select(LEVEL2_DATA_NAMES[["StubTechFractCalPrice"]]) -> L202.StubTechFractCalPrice_an

    # print(L202.StubTechFractProd_an)
    # print(L202.StubTechFractCalPrice_an)

    #secondary outputs
    A_an_rsrc_info %>%
      repeat_add_columns(A_regions) %>%
      select(region, resource, output.unit = "output-unit", price.unit = "price-unit", market) %>%
      mutate(market = region) -> L202.Rsrc_an
...
#     print(L202.RsrcPrice_an)

    ### END CODE FROM zenergy
    ...
    L202.StubTechFractSecOut_an %>%
      ...
      add_legacy_name("L202.StubTechFractSecOut_an") %>%
      add_precursors("aglu/A_an_technology", "aglu/A_an_secout", "common/GCAM_region_names") ->
      L202.StubTechFractSecOut_an

    L202.StubTechFractProd_an %>%
      add_title("Price and production fraction for secondary outputs") %>%
      add_units("1975$, fraction") %>%
      add_comments("Prices from external data") %>%
      add_legacy_name("L202.StubTechFractProd_an") %>%
      add_precursors("aglu/A_an_secout_prices") ->
      L202.StubTechFractProd_an

    L202.StubTechFractCalPrice_an %>%
      add_title("Calibrated prices of secondary outputs of animal raising operations") %>%
      add_units("1975$/kg") %>%
    ...
    L202.Rsrc_an %>%
      ...
      add_units("unitless") %>%
      add_comments("A_an_rsrc_info written out to regions with secondary feed output") %>%
      add_legacy_name("L202.Rsrc_an") %>%
      add_precursors("aglu/A_an_rsrc_info") ->
      L202.Rsrc_an

    L202.RsrcPrice_an %>%
      add_title("Resource price for animal secondary output") %>%
      add_units("1975$") %>%
      add_comments("A_an_rsrc_info interpolated to all historical model time periods") %>%
      add_legacy_name("L221.RsrcPrice_en") %>%
      same_precursors_as(L202.StubTechFractCalPrice_an) ->
      L202.RsrcPrice_an
