...
module_aglu_an_input_xml <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L202.Rsrc_an <- get_data(all_data, "L202.Rsrc_an")
    L202.StubTechFractSecOut_an <- get_data(all_data, "L202.StubTechFractSecOut_an")
    L202.StubTechFractProd_an <- get_data(all_data, "L202.StubTechFractProd_an")
    L202.StubTechFractCalPrice_an <- get_data(all_data, "L202.StubTechFractCalPrice_an")
    L202.RsrcPrice_an <- get_data(all_data, "L202.RsrcPrice_an")
    ...
    create_xml("an_input.xml") %>%
      ...
      # add secondary out here
      add_xml_data(L202.StubTechFractSecOut_an, "StubTechFractSecOut") %>%
      add_xml_data(L202.StubTechFractProd_an, "StubTechFractProd") %>%
      add_xml_data(L202.StubTechFractCalPrice_an, "StubTechFractCalPrice") %>%
      add_xml_data(L202.StubTechProd_in, "StubTechProd") %>%
      # add rsrc info here
      add_xml_data(L202.Rsrc_an, "Rsrc") %>%
      add_xml_data(L202.RsrcPrice_an, "RsrcPrice") %>%
