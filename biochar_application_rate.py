...
def get_app_rate(row, product, biochar, nutrient, year, land):
    ...
    # Mt manure * Mt biochar/Mt manure * Mt nutrient/Mt biochar * 1e6 Mg / Mt) / (thousand km2 * hectares/thousand km2)
    return row[year] * yields * biochar_to_nutrient * 1e6 / (row[land] * 100000) if row[land] != 0 else np.nan
...
if __name__ == '__main__':
    ...
    released_land = released_land[released_land['2080'] > 0]
    released_land = data_manipulation.group(released_land, "GCAM") # group by GCAM data
    merged = pd.merge(released_supply, released_land, "left", left_on="GCAM", right_on="GCAM",
                      suffixes=("", "_land"))
    ...
    C_rates = merged.groupby("GCAM")[[str(i)+"_C" for i in c.GCAMConstants.biochar_x]].sum().reset_index()
    P_rates = merged.groupby("GCAM")[[str(i)+"_P" for i in c.GCAMConstants.biochar_x]].sum().reset_index()
    K_rates = merged.groupby("GCAM")[[str(i)+"_K" for i in c.GCAMConstants.biochar_x]].sum().reset_index()
    ...
    plot_world(C_rates, c.GCAMConstants.biochar_x, "C application rates (Mg/ha)")

    # map P application rates on cropland, bio-energy land, and both
    plot_world(P_rates, c.GCAMConstants.biochar_x, "P application rates (Mg/ha)")

    # map K application rates on cropland, bio-energy land, and both
    plot_world(K_rates, c.GCAMConstants.biochar_x, "K application rates (Mg/ha)")

    # graph histogram of C application rates across all years and regions
    C_rates = C_rates[[str(i) for i in c.GCAMConstants.biochar_x]]
    ...
    P_rates = P_rates[[str(i) for i in c.GCAMConstants.biochar_x]]
    ...
    K_rates = K_rates[[str(i) for i in c.GCAMConstants.biochar_x]]
