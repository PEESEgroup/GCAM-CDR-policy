...
def standard_plots(nonBaselineScenario, RCP):
    ...
    correlation_biomass_price_supply = pd.merge(biomass_price, perc_diff_biomass, "left", on=["GCAM", "SSP"],
                                                suffixes=("_left", "_right"))
    plotting.plot_correlation(correlation_biomass_price_supply, c.GCAMConstants.plotting_x, ["SSP2"],
                              "% change in biomass price", "% change in biomass supply", "upper left")
    ...
    #         "2060-2065", "2065-2070", "2070-2075", "2075-2080", "2080-2085", "2085-2090", "2090-2095", "2095-2100"]
    plotting.plot_correlogram(pyrolysis_price, c.GCAMConstants.SSPs, cols,
...
def prices(nonBaselineScenario, RCP, SSP):
    ...
    for products in ["regional beef", "regional dairy", "regional wheat"]:
        for energy in ["refined liquids enduse", "crude oil", "electricity", "natural gas"]:
            ...
            perc_diff_food = data_manipulation.percent_difference(released_food_price, pyrolysis_food_price,
                                                                  ["SSP", "sector", "GCAM"])
            ...
            stats.plot_correlation(correlation_food_energy_pyrolysis, years, SSP,
                                   "pyrolysis " + str(products) + " price", "pyrolysis " + energy + " price",
                                   "upper left")
            ...
            stats.plot_correlation(correlation_food_energy_diff, years, SSP,
                                   "% change in " + str(products) + " price", "% change in " + energy + " price",
                                   "upper left")
...
def carbon_prices(nonBaselineScenario, RCP, SSP):
    ...
    released_CO2_price = released_CO2_price[released_CO2_price[['product']].isin(products).any(axis=1)]
    pyrolysis_CO2_price = pyrolysis_CO2_price[pyrolysis_CO2_price[['product']].isin(products).any(axis=1)]
    flat_diff_CO2_price = data_manipulation.flat_difference(released_CO2_price, pyrolysis_CO2_price,
                                                            ["SSP", "product", "GCAM"])
...
def carbon_price_correlation(nonBaselineScenario, RCP, SSP):
    """
    Tests for carbon price correlation
    :param nonBaselineScenario: the scenario being compared to the released model
    :param RCP: the RCP pathway (or baseline)
    ...
    pyrolysis_CO2_price = pd.read_csv(
        "data/gcam_out/" + str(nonBaselineScenario) + "/" + RCP + "/CO2_prices.csv")  # prices_of_all_markets
    ...
    for products in ["regional beef", "regional dairy", "regional wheat"]:
        ...
        released_res = stats.calc_price_linkage(released_CO2_price, released_food_price, SSP, 2050)
        pyrolysis_res = stats.calc_price_linkage(pyrolysis_CO2_price, pyrolysis_food_price, SSP, 2050)
        plotting.plot_price_coefficients(pyrolysis_res, released_res,
                                         "price coefficients for " + products + " and carbon price in " + SSP[0])


def price_correlation(nonBaselineScenario, RCP, SSP):
    """
    Measures price linkages between the released and pyrolysis version of the model
    :param nonBaselineScenario: the scenario being compared to the released model
    :param RCP: the RCP pathway (or baseline)
    :param SSP: the SSP pathway
    :return: N/A
